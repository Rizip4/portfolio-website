...
const borderRadiusMap: Record<SizeVariant, number> = { sm: 16, default: 18, lg: 20 };

const FlowButton = React.forwardRef<HTMLButtonElement, FlowButtonProps>(
  ({ children, size = "default", borderColor = "var(--rotating-border-color)", className, asChild = false, ...props }, ref) => {
      ...
      <>
        ...
        <div className="relative inline-block group pointer-events-none">
          ...
          >
            ...
            >
              <path d={createRoundedRectPath(dimensions.width, dimensions.height, radius)}
          ...
          <Comp ref={buttonRef}
            className={cn(
              "relative z-0 [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 shrink-0 [&_svg]:shrink-0 pointer-events-auto cursor-pointer inline-flex items-center justify-center gap-2 whitespace-nowrap font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 bg-neutral-100 dark:bg-neutral-800 text-[#0C0C0C] dark:text-white hover:bg-transparent",
